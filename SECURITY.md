...
- 消耗供应商额度的搜索路径使用 Railway API 实例内按 IP 限流作为第一道防线；它不是跨实例全域总量限制，因此供应商账户用量上限、剩余额度检查和应用级硬停止才是最终保护；V2 若开放多副本或公开高流量，必须先增加经过并发验证的全局原子配额；
...
- 生产只允许 HTTPS；
- Web 由 Next.js、API 由 Fastify 显式设置防嗅探、禁止嵌入、Referrer、权限和跨源窗口隔离响应头；Netlify 配置保留 Web 侧同类兜底；
- 管理端启用前必须有强认证与最小权限。
