...
扩展已在 Edge 开发人员模式加载。重载并刷新本地桥接页面后，`PEK → SHA` 同次查询五个来源全部成功：携程返回 30 个结构化响应 Offer、去哪儿 20 个卡、同程 30 个卡、FlyAI 10 个、SerpApi 2 个；携程报价标记为 `provider_response_verified`。飞猪页面 Companion 失败时由正式 FlyAI Connector 接管，不再降低来源覆盖。
