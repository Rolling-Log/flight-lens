...
初次真实检索记录时间：2026-07-31 15:52 CST；本轮候选复核时间：2026-08-04 12:15 CST。

- Git 运行时候选提交：`dcf9fbf`（包含附近始发机场执行、双输入一致性、Netlify 同域 API、有界搜索运行时、免费额度防滥用、不合格报价隔离与来源状态披露修复）；
- Netlify 项目：`flight-lens-staging`，当前候选 Deploy：`6a71673f0d395a1b61f0561c`；
...
- 延迟边界：往返查询曾在 Netlify Function 的 30 秒硬限制处终止。候选移除较慢且非必要的 `deep_search=true`；后续附近机场真实样本又暴露 Neon 审计写入没有独立超时、可能阻塞已经完成的航班结果。当前 Connector 默认预算收紧为 20 秒，审计预算为 3 秒并以 `AUDIT_PERSIST_TIMEOUT` 降级披露，为标准化和响应封装保留确定空间；
