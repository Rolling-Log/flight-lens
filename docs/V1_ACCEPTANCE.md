...
初次真实检索记录时间：2026-07-31 15:52 CST；本轮候选复核时间：2026-08-04 10:57 CST。

- Git 候选提交：`3760af9`（包含 SerpApi 延迟边界、双输入一致性和 Netlify 同域 API 修复）；
- Netlify 项目：`flight-lens-staging`，当前候选 Deploy：`6a7154af01e4babc3c3a4e3d`；
...
- 回归：`pnpm check` 全部通过（49 项测试、类型检查、Lint、生产构建）；Playwright 桌面与移动端端到端测试 6/6 通过；
- 凭据：数据库密码在配置过程中完成轮换，旧连接串失效；Netlify 中的 `DATABASE_URL` 与 `SERPAPI_API_KEY` 均为 secret；迁移后系统剪贴板已清空。
- 回滚：在隔离 worktree 重建提交 `5503c45`，部署为 `6a71418ce8ca6745e61e690f`，候选 URL 的页面与 API 均为 HTTP 200，数据库、Connector、限流和安全响应头正常；随后恢复候选 `6a7141dbcd73917640ef8e39` 并再次通过页面与 API 健康检查。延迟修复候选 `6a71465d6ca2a08f25cd29d5` 与同域 API 修复候选 `6a7154af01e4babc3c3a4e3d` 随后部署并验证为 `ready`。整个演练未触碰 Production。
