...
- 免费额度防滥用：最初仅用 `netlify.toml` 自重写和 Fastify 内存限流，Staging 前三次无效请求均返回 400，证明请求会分散到不同 serverless 实例且边缘规则未生效；继续观察到第六次才由单个 API 实例返回 429。候选随即改为同一 Netlify 项目内的专用 Edge Function，并保留 API 内层限流。最终用不触发 Connector 的无效请求复核，状态依次为 400、400、429，第三次携带 `Retry-After: 60`；整个限流测试未消耗 SerpApi 额度。免费计划只能按域名+IP 聚合，不能提供企业版全域总量限制，因此分布式多 IP 滥用仍是已披露残余风险，供应商 `$0` 套餐与剩余额度硬停止是最后保护；
