...
Playwright E2E 在桌面 Chromium 和移动端 Chromium 中验证：对话解析、条件确认、表单回填、搜索、来源覆盖、跨日、六种排序入口、来源跳转精度和不售票声明。测试使用本地固定响应，不消耗供应商配额；CI 在 `pnpm check` 后执行 `pnpm test:e2e`。
...
Wego 契约测试必须覆盖 OAuth、单次用户搜索、稳定计数轮询、跨批次合并、实际 Provider、CNY 总价、官方 handoff 白名单与灵活日期不扩增搜索。
