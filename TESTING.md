...
Playwright E2E 在桌面 Chromium 和移动端 Chromium 中验证：完整对话解析、缺失条件的保守追问与部分回填、条件确认、表单同步、搜索、来源覆盖、跨日、六种排序入口、来源跳转精度和不售票声明。测试使用本地固定响应，不消耗供应商配额；CI 在 `pnpm check` 后执行 `pnpm test:e2e`。
