...
- Netlify Function 当前同步执行上限为 60 秒。Connector 仍在 20 秒主动超时，Neon 审计写入最多等待 3 秒并可降级披露，为平台变化和响应封装保留充足空间；SerpApi 使用默认快速模式并保留 `no_cache=true`，避免将过期缓存当实时价；落地页仍承担最终可售性与价格复核；
- SerpApi 除 `$0` 套餐和剩余额度检查外，还使用账户级月度 credit 硬上限：代码默认 200/250，Staging 为 100。门禁依据供应商账户的全局用量而非单个 serverless 实例；并发请求仍可能在读取用量与实际扣减之间产生小幅竞态，因此 Staging 预留 150 credits 安全余量且不做真实搜索压测；
