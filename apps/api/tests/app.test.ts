...
test("health discloses connector release readiness", async () => {
  const app = await buildApp({ config, connectors: [], auditStore: null });
  const response = await app.inject({ method: "GET", url: "/health" });
  assert.equal(response.statusCode, 200);
  assert.deepEqual(response.json().connectors, { configured: 0, releaseMinimum: 2 });
  await app.close();
});
...
test("returns transparent coverage for a configured empty source", async () => {
  ...
  const app = await buildApp({ config, connectors: [connector], auditStore: null });
  const response = await app.inject({
    method: "POST",
    url: "/v1/searches",
    payload: validIntent,
